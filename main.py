...
# ---------- Установка SteamCMD (при первом запуске) ----------
def ensure_steamcmd_installed():
    if STEAMCMD_EXE.exists():
        log_message("SteamCMD уже установлен.")
        return True
...
def fetch_mod_details(app_id: str, mod_id: str) -> dict | None:
    ...
    try:
        ...
        if details.get("result") != 1:
            log_message(f"Мод {mod_id} не найден (result={details.get('result')}).", "WARN")
            return None
        # Не проверяем принадлежность к app_id, оставляем универсальность
        return {
...
def download_thread(app_id, mod_id, folder_name, callback):
    ...
    try:
        ...
        if "ERROR!" in stdout or "ERROR!" in stderr or proc.returncode != 0:
            if "caller chunk indicies out of date" in stdout or "Unexpected result 11" in stdout:
...
class App(tk.Tk):
    def __init__(self):
        ...
        tk.Label(app_frame, text="AppID:", fg="white", bg="#1e1e1e", font=("Segoe UI", 9)).grid(row=0, column=0, padx=5)
        self.appid_entry = tk.Entry(app_frame, width=10, font=("Segoe UI", 11), bg="#2d2d2d", fg="white", insertbackground="white")
        self.appid_entry.grid(row=0, column=1, padx=5)
        self.appid_entry.insert(0, "1118200")  # People Playground по умолчанию
        # Список популярных игр (необязательно)
        tk.Label(app_frame, text="Популярные:", fg="#aaaaaa", bg="#1e1e1e", font=("Segoe UI", 8)).grid(row=0, column=2, padx=(15,5))
        ...
        self.preset_combo.grid(row=0, column=3, padx=5)
        ...
        # Кнопка "Искать" и поле ID мода
        mod_frame = tk.LabelFrame(main_frame, text="Мод (Workshop ID)", fg="#cccccc", bg="#1e1e1e",
                                  font=("Segoe UI", 9, "bold"), padx=10, pady=10)
        mod_frame.pack(fill=tk.X, pady=(0,10))
        tk.Label(mod_frame, text="Mod ID:", fg="white", bg="#1e1e1e", font=("Segoe UI", 9)).grid(row=0, column=0, padx=5)
        self.modid_entry = tk.Entry(mod_frame, width=25, font=("Segoe UI", 11), bg="#2d2d2d", fg="white", insertbackground="white")
        self.modid_entry.grid(row=0, column=1, padx=5)
        self.search_btn = tk.Button(mod_frame, text="🔍 Найти", command=self.search_mod,
                                    bg="#e05a3c", fg="white", font=("Segoe UI", 10, "bold"),
                                    activebackground="#c0392b", relief=tk.FLAT, padx=15)
        self.search_btn.grid(row=0, column=2, padx=10)

        # Панель информации и превью
        ...
        self.info_text = tk.StringVar(value="Введите AppID игры и ID мода, затем нажмите «Найти».")
        tk.Label(info_text_frame, textvariable=self.info_text, fg="white", bg="#1e1e1e",
                 font=("Segoe UI", 9), justify=tk.LEFT, wraplength=400).pack(anchor="nw")
        ...
        self.log_btn = tk.Button(btn_frame, text="📄 Лог-файл", command=self.open_log,
                                 bg="#555", fg="white", font=("Segoe UI", 9), relief=tk.FLAT)
        self.log_btn.pack(side=tk.LEFT, padx=2)
        self.folder_btn = tk.Button(btn_frame, text="📁 Папка загрузок", command=self.open_mods_folder,
                                    bg="#555", fg="white", font=("Segoe UI", 9), relief=tk.FLAT)
        ...
        # Статус-бар
        self.status_var = tk.StringVar(value="Готов к работе (анонимный вход)")
    ...
    def load_preview_image(self, url):
        """Загружает и отображает превью-картинку в preview_label"""
        if not url:
        ...
        try:
            ...
            log_message(f"Ошибка загрузки превью: {e}", "WARN")
    ...
    def search_mod(self):
        ...
        if not app_id.isdigit() or not mod_id.isdigit():
            ...
            return
        self.set_status("Поиск информации о моде...")
    ...
    def open_preview(self):
        ...
        elif self.current_mod and self.current_mod.get("preview_url"):
            webbrowser.open(self.current_mod["preview_url"])
    ...
    def download_mod(self):
        ...
        dialog.grab_set()
        tk.Label(dialog, text="Выберите имя папки для сохранения:", fg="white", bg="#2d2d2d",
